...
import { readDb, withDbUpdate, createAppError, getMockSession, setMockSession, clearMockSession, getVisibleKitchens, touchUser } from "./_repoHelpers";
...
export async function login({ email }) {
  ...
  await withDbUpdate((db) => {
    ...
    let user = findUserByEmail(db, normalizedEmail);
    const derivedRole = deriveRoleFromEmail(normalizedEmail);
      ...
      db.users.push(user);
    } else if (isTestEmail && user.roleSource !== "manual") {
      if (!user.roleSource) user.roleSource = "email";
      if (user.role !== derivedRole) user.role = derivedRole;
    }

    if (isTestEmail && user.roleSource !== "manual") {
      ensureDefaultScopeForRole(db, user);
    }
