...
import { useAssignUserRoleMutation, useUpdateUserAccessMutation } from "../api/useOrganizationMutations";
...
export default function UserAccessModal({
  ...
  const roleOptions = useMemo(() => {
    ...
    return [
      ...
      ...ROLE_OPTIONS.filter((option) => allowedRoles.has(option.value)).map((option) => ({
        value: option.value,
        label: option.label,
      })),
    ];
  ...
  return (
    <Modal
      ...
      title="User access"
      description="Manage role assignment and restaurant access within scope. Founder/Owner is data-only and cannot operate lists."
    >
      {user ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-[18px] bg-[var(--surface-soft)] p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              {user.joinedViaCode ? <Badge variant="success">Joined via code</Badge> : null}
            </div>
          </div>
